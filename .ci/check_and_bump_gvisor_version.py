...
def commit_and_push_to_branch(git_helper:gitutil.GitHelper, files_to_commit: list[str], branch: str, commit_msg: str) -> str:
    """
    Commit and push changes to a specific branch.
    """
    try:
        for f in files_to_commit:
            git_helper.repo.index.add(f)
...
def file_github_pull_request_with_changes(repo_dir: str, upstream_gvisor_version: str):

    git_helper = get_git_helper(owner=GARDENER_EXTENSION_GVISOR_OWNER, repository=GARDENER_EXTENSION_GVISOR_REPOSITORY, github_repo_url=GARDENER_EXTENSION_GVISOR_REPO_URL, repo_dir=repo_dir)
    ...
    branch_name = f"bump-gvisor/{upstream_gvisor_version}"

    if branch_exists(git_helper=git_helper, branch=branch_name):
        raise RuntimeError(f"branch {branch_name} already exists")
    
    commit_message = f"bump gVisor version to {upstream_gvisor_version}"

    changed_files = [
        "GVISOR_VERSION"
    ]

    success_message = commit_and_push_to_branch(git_helper, changed_files, branch_name, commit_message)
    if success_message != "CommitPushed":
        raise RuntimeError(f"Failed to commit and push changes to branch {branch_name}")

    pr_title = commit_message

    body = f"""**What this PR does / why we need it**:

Bumps the version of the included gVisor binary to {upstream_gvisor_version}
...
```other operator
The gVisor binary was updated to release {upstream_gvisor_version}
```
"""

    create_pull_request(git_helper=git_helper, org=GARDENER_EXTENSION_GVISOR_OWNER, name=GARDENER_EXTENSION_GVISOR_REPOSITORY, pr_title=pr_title, pr_body=body, branch=branch_name)


if __name__ == "__main__":
    repo_root = get_repo_root()
    current_version = get_current_version(repo_root)
    upstream_version = get_upstream_version_from_tags(organization="google", repository="gvisor")
    ...
    if upstream_version > current_version:
        ...
        file_github_pull_request_with_changes(repo_dir=repo_root, upstream_gvisor_version=upstream_version)
